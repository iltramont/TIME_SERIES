...
print(metrics_df)
```
Looking at the metrics, it seems tha the best model is the Holt-Winters with a multiplicative approach. If we look at the ME and MPE we see how HW-additive performs better, this means that predictions are 'more centered' than the HW-multiplicative one.
...
If we repeat the process with monthy data, we obtain a period of 12, as expected.
...
for (var_name in colnames(zoo_data)) {
  plot(var_forecast[[var_name]][,1], lwd=2)
}
```
